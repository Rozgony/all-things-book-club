...
- [x] consider security improvements
  - [x] Token expiry (7 days)
  - [x] make sure we delete invites after accepted
- [x] Make create user flow better
  - [x] currently: "Sign up succeeded but no session was returned" because email address confirmation is required. Does it have to be that way?
  - [x] why is there such a delay after clicking the email accept button?
- [x] Make email link a clickable button
- [x] Is there a way to not be able to see the email text in Resend?
- [x] Why are chapters now wrapped by the public key instead of the user key? -- Removed
- [x] Better welcome screen than "mkschultz@proton.me has invited you to an All Things Book Club chapter."
- [x] No one's chapters are loading, not for the creator or the invited.