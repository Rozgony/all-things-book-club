...
Replace the current symmetric chapter-key wrapping (PBKDF2 `userKey` → AES-GCM) with asymmetric ECDH wrapping (Argon2id → X25519 keypair). A one-time invite secret embedded in the email link's `#hash` fragment hands off the chapter key to new users fully asynchronously — the inviter does nothing after clicking "send."
...
- **PBKDF2 `userKey` kept** — still used for user profile blob encryption only. Coexists without migration.
...
- [x] **2.3** Add `GET /api/users/by-email?email=X` — returns `{ id, publicKey }`; requires auth; caller must share a chapter with the target (prevents enumeration)
...
- [x] **3.1** Update `frontend/src/pages/Home/LoginPage.tsx` — after existing PBKDF2 step, add in parallel:
...
- [x] **5.2** On submit — first call `GET /api/users/by-email?email=X` to determine which path:
  - **Existing user** (account found): `wrapChapterKeyForRecipient(chapterKey, inviteePubKey)` → `POST /api/chapters/{id}/members` with the ECDH-wrapped key and `ephemeralPublicKey` directly; send notification email separately. No invite secret, no URL hash — the key is already waiting when they next log in.
  - **New user** (404): generate `inviteSecret = crypto.getRandomValues(new Uint8Array(32))`; `wrapChapterKeyWithSecret(chapterKey, inviteSecret)`; `POST /api/chapters/{id}/invites` with `{ invitedEmail, encryptedChapterKey, keyNonce, inviteSecretBase64url: base64url(inviteSecret) }` — backend generates the real token, constructs the full URL, and sends the email. The `inviteSecret` is never persisted by the backend, only handled in-memory during email dispatch. See trust boundary note in Phase 2.4.
  - **TOFU note (existing user path):** the inviter has no out-of-band way to verify the public key returned by the server belongs to the intended person. A compromised server could substitute a rogue key. This is an inherent limitation of server-mediated key distribution — acceptable for this threat model but worth documenting.
...
- [ ] Invite existing user → they log in → can read chapter with zero admin follow-up action
...
- **In scope:** X25519 keypair derivation, ECDH chapter key wrapping, new-user invite via URL hash secret, existing-user invite via direct ECDH wrap, login/signup key setup, schema migrations
- **Out of scope:** Public topics layer, password-change key rotation, multi-device explicit re-registration
